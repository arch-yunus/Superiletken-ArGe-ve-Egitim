...
Bu modülde, süperiletkenliğin kuantum mekaniksel temeli olan **BCS (Bardeen-Cooper-Schrieffer) Teorisi** ve **Cooper Çiftleri** oluşumu analitik olarak incelenmektedir.
...
## 3. BCS Taban Durumu Dalga Fonksiyonu
...
## 4. Kuantum Enerji Boşluğu (Energy Gap - $\Delta$)
...
Bu bağıntı, zayıf kuplajlı (weak-coupling) geleneksel süperiletkenler için son derece hassas sonuçlar verir.

---

## 5. Termodinamik Öngörüler: Özgül Isı Sıçraması
...
```
            Elektronik Özgül Isı Kapasitesi C_es / (γ T_c)
            
            3.0 ┼               /│
                │              / │  Süperiletken Durum (C_es ~ e^(-Δ/k_BT))
            2.0 ┼             /  │
                │            /   │
            1.43┼───────────/    │
                │          /     │
            1.0 ┼─────────/──────┼──────  Normal Durum (C_en = γ T)
                │        /       │
                └───────┴────────┴───────
                       0.5      1.0 (T/T_c)
```
...
1. **Kritik Sıcaklıktaki Sıçrama ($T = T_c$):**
   ...
   Bu sıçrama, Cooper çiftlerinin oluşmaya başlamasıyla entropinin ani bir şekilde düşmesinden kaynaklanır.
...
3. Ketterson, J. B., & Song, S. N. (1999). *Superconductivity*. Cambridge University Press.
